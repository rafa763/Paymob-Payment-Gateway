...
import { pay } from "./initiTrx.js";
import { createHmac } from "crypto";
import { order_cart, billing_data } from "./data.js";
...
server.get("/pay", async (req, res, next) => {
  try {
    // Perform the payment
    const amount_cents = req.query.amount_cents;
...
import { getTransactionById } from "./getTrx.js";
server.get('/trx', async (req, res, next) => {
...
// void a transaction
import { voidTransaction } from "./voidTrx.js";
server.get("/void", async (req, res, next) => {
    const transactionId = req.query.transactionId;
...
import { refundTransaction } from "./refundTrx.js";
server.get("/refund", async (req, res, next) => {
    const transactionId = req.query.transactionId;
    const transaction = await getTransactionById(transactionId);
    const refunded = await refundTransaction(transactionId, transaction.amount_cents);
    res.send(refunded);
});

// verify from paymob server callback reuqest
